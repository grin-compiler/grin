...
\begin{document}
	...
	In this section, we will discuss the benefits and challenges of compiling GRIN to LLVM.
	...
	However, in some cases the monomorphic type inference algorithm presented above is not sufficient. For example, the Glasgow Haskell Compiler has polymorphic primitive operations. This means, that despite GRIN being a monomorphic language, certain compiler front ends can introduce polymorphic functions to GRIN programs. To resolve this problem, we need to further extend the heap points-to analysis. The algorithm now needs a table of external functions with their respective type information. These functions \emph{can} be polymorphic, hence they need special treatment during the analysis. While propagating information through these functions, the algorithm will introduce new abstract heap locations for each unique type variable, and it will keep track of its possible concrete types. Fortunately, this can be accomplished by the same data-flow operations the algorithm already uses.
