...
import GHC.Generics
import Grin.ExtendedSyntax.Grin hiding (Def)
...
import qualified Test.ExtendedSyntax.New.Grammar as G
...
changeLast e r@(ECase{})   = EBind (SBlock r) (VarPat "cl") e
changeLast e r             = EBind r (VarPat "cl") e
...
instance Arbitrary G.BPat where arbitrary = genericArbitraryU
    ...
    _ -> tryoutF
          [ -- (10, G.SExp <$> (solve (SExp (NoEff t))))
            (80, do (t', se) <-
                    ...
                    newVar t' $ \n -> do -- TODO: Gen LPat
                      rest <- solve (Exp [] t)
                      pure (G.EBind se (G.VarPat (G.Name n)) rest))
          , (20, gCase [] t)
          ]
  (e:es) -> case n of
