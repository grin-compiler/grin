  ...
  folder = \case
        ...
        AsPat v val -> do
          ...
          case val of
            ...
            ConstTagNode tag args -> do
              ...
              bindInstructions <- codeGenBlock_ $ forM (zip [1..] args) $ \(idx, name) -> do
                argReg <- newReg
                addReg name argReg
            ...
            _ -> error $ "LVA: unsupported value for as-pattern " ++ show (PP bPat)
    ...
    ECaseF scrut alts_ -> do
      ...
      forM_ alts $ \(A cpat altM) -> do
        ...
        case cpat of
          ...
          LitPat lit -> do
            -- NOTE: should be altResultRegister
            caseResultReg `isLiveThenM`         setBasicValLive scrutReg
            caseResultReg `hasSideEffectsThenM` setBasicValLive scrutReg
          ...
          _ -> error $ "LVA does not support the following case pattern: " ++ show (PP cpat)
    ...
    SAppF f args -> do
      ...
      case f of
        ...
        Ext name -> do  -- external
          mExt <- getExternal name
          case mExt of
            Just ext
              | eEffectful ext -> do mapM_ setBasicValLive argRegs
            ...
            Nothing -> error $ "LVA could not find external: " ++ show f
    ...
    SUpdateF p v -> do
      ...
      -- setting pointer liveness
      emit $ varReg `isLiveThen` [setBasicValLiveInst addressReg]

      R <$> newReg

    SBlockF exp -> exp

codeGenAlt :: Name -> IR.Reg ->
              ...
              (IR.Reg -> Name -> CG ()) ->
              CG [IR.Instruction]
codeGenAlt name reg restrict before altM after restore =
  codeGenBlock_ $ do
    altReg <- restrict reg name
    before altReg
    altResult <- altM
    after altResult
    restore reg name
